...
public class Ui_utils {
   ...
   /* public static String get_filelen(Files files)
    ...
    public static void url_to_tp(String url)
    {
            ...
            public void run()
            {
                try
                {
                    ...
                    if(url.contains("www.")){
                        ...
                        address = InetAddress.getByName("www." + url);
                    }
                    ndk_wrapper.instance().avsz_init(address.getHostAddress().toString(), (short) 1220, SPUtils.getString(Ui_utils.get_context(),
                            user_name), SPUtils.getString(Ui_utils.get_context(), pass_word));
                    L.d(TAG,"url_to_tp" + address.getHostAddress().toString());
                } catch (UnknownHostException e)
                {
                    e.printStackTrace();
                    L.d("BEACON_UTIL", "get ip error" + e.getMessage());
                }
            }
        }).start();
